...
public class CommandTransaction implements Command {
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException,
        ...
        if (act == null) act = ""; // error is thrown at the end of switch case
        LOGGER.info("Session.Client:" + client + "; action:" + act);

        if (!CommandUtils.accountBelongsToUser(req, dtAccountId, LOGGER))
            return page;
        ...
        switch (act) {
            case ("replenish"): // Same operation as transfer, except dt_account is user's acct, amount is negative
                try {
                    trId = SERVICE.getUser().replenishAccount(dtAccountId, trfAmount, Integer.parseInt(RB_BANK
                            .getString("CASH_ACCOUNT_ID")));
                } catch (ExceptionDAO e) {
                    LOGGER.error(e);
                }
                break;

            // TODO : block account if comission cannot be charged - or try another user's account
            ...
            case ("make_payment"): /* Make payment and Also subtract fee for this transfer according to fee.plan */
                ...
                Account comissionsAcct = SERVICE.getUser().getAccountById(Integer.parseInt(RB_BANK.getString
                        ("COMISSIONS_ACCOUNT_ID")));
                BigDecimal comission = BigDecimal.valueOf(fee.getTransferFee());
                SERVICE.getUser().makePayment(dtAccount, comissionsAcct.getNumber(), comission, "Comission for " +
                        "Transaction # " + trId);
