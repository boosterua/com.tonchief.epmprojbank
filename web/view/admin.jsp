...
<div class="container">
  <div class="row">
    <div class="col-md-1 verticaltext">
      <h1 class="verticaltext_content display-4" style="text-shadow: 3px 3px 6px #CCC; color:#FFF9F9;">
          admin&nbsp;module</h1>
    </div>
    ...
    <div class="col-md-10 container-fluid">    <%--style="border:2px solid red;"--%>
      ...
      <c:if test="${action==\"index\"}">
        ...
        <a href="<%=request.getContextPath()%>/bank/?command=admin&action=show_clients_by_role&role=0">
          <button type="button" class="btn btn-sm btn-default teal darker waves-effect waves-light"
              aria-label="Left Align">
            <span class="glyphicon glyphicon-align-left" aria-hidden="true"></span>
            <i class="fa-li fa fa-check-square"></i><fmt:message key="LNK_SHOW_NEW_CLIENTS"/>
          </button>
        </a><br>
        ...
        <a href="<%=request.getContextPath()%>/bank/?command=admin&action=show_clients_with_blocked_accounts">
          <button type="button" class="btn btn-sm  btn-default  teal darker waves-effect waves-light"
              aria-label="Left Align">
            <span class="glyphicon glyphicon-align-left" aria-hidden="true"></span>
            <i class="fa-li fa fa-check-square"></i><fmt:message key="LNK_SHOW_BLOCKED_CLIENTS"/>
          </button>
        </a><br>
        ...
        <a href="<%=request.getContextPath()%>/bank/?command=admin&action=show_all_clients">
          <button type="button" class="btn btn-sm  btn-default teal darker waves-effect waves-light"
              aria-label="Left Align">
            <span class="glyphicon glyphicon-align-left" aria-hidden="true"></span>
            <i class="fa-li fa fa-check-square"></i><fmt:message key="LNK_SHOW_ALL_CLIENTS"/>
          </button>
        </a><br>
          ...
          <table class="table table-striped table-hover table-sm table-info z-depth-3"
            ...
            <c:forEach var="cl" items="${clientList}">
              <tr class="text-center">
                ...
                <td>${cl.getEmail()}</td>
                <td class="text-left">
                  <nobr>${cl.getAccount().getName()}
                    <c:if test="${cl.getAccount().getId()==0}">*</c:if>
                    <c:if test="${cl.getAccount().getBlocked()==true}">&nbsp;<span
                        class="btn btn-deep-orange btn-sm"
                        style="line-height:2px;padding:7px 4px;margin:0;"><fmt:message
                        key="BLOCKED" bundle="${lang}"/></span></nobr>
                  </c:if>
      ...
      </c:if>        <%--/c:if show_clients_by_role--%>
